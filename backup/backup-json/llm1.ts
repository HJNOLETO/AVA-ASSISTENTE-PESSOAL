...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  const provider =
    params.provider ||
    (process.env.LLM_PROVIDER as "forge" | "ollama" | undefined) ||
    "forge";
  ...
  if (provider === "ollama") {
    ...
    const chatMessages = messages.map((m) => {
      ...
      const images: string[] = parts
        ...
        .map((p: any) => {
          ...
          // Accept data URLs: data:<mime>;base64,<payload>
    ...
    const response = await fetch(url, {
      ...
      headers: {
        ...
        ...(params.ollamaAuthToken
          ? { authorization: `Bearer ${params.ollamaAuthToken}` }
          : {}),
      },
      body: JSON.stringify({
