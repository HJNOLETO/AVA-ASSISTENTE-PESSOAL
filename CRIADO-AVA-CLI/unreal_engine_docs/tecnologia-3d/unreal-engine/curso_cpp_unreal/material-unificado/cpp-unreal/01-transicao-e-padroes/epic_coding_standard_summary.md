...
| **Memória/Ponteiros** | Ponteiros brutos (`*`), `std::shared_ptr`, `std::unique_ptr`. Gerenciamento manual ou via _smart pointers_. | Ponteiros brutos, mas principalmente _smart pointers_ da UE (`TSharedPtr`, `TWeakObjectPtr`) e ponteiros especiais para `UObject` (`UPROPERTY` e `TObjectPtr`).  |
