...
- Código C++ (Editor-only, UE 5.5+ segundo o repositório original)
...
> **Portátil:** A pasta `UnrealMCP/` pode ser copiada para qualquer outro projeto UE5. Basta colar em `<Projeto>/Plugins/UnrealMCP/` e abrir o `.uproject`.
> 
> ⚠️ **Compatibilidade:** Plugins testado e compilado no **UE 5.6** neste projeto. O repositório original do Flopperam especifica **UE 5.5+** como requisito mínimo.
...
### Passo 1: Abrir o ProjetoGTA no UE5 e ativar os plugins

Abra `ProjetoGTA.uproject` — o editor vai detectar o novo plugin `UnrealMCP/` e compilá-lo automaticamente.

Após abrir, verifique em **Edit → Plugins**:

| Plugin | Necessário para |
|--------|----------------|
| **UnrealMCP** (instalado) → Enabled ✓ | Canal `unreal_mcp` (TCP :55557) |
| **Remote Control API** (nativo do UE5) → Enabled ✓ | Canal `unreal_ops` (HTTP :30010) |

> O Remote Control API é um plugin nativo da Epic, não parte do Flopperam MCP. É necessário apenas para o `unreal_ops` (Python, console, screenshot).
