...
{
    Result->SetBoolField(TEXT("success"), false);
    Result->SetStringField(TEXT("error"), TEXT("Failed to create variable"));
    }
...
TSharedPtr<FJsonObject> FBPVariables::SetVariableProperties(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (Params->HasField(TEXT("default_value")))
    {
        ...
        UpdatedProperties->SetStringField(TEXT("default_value"), TEXT("updated"));
    }
    ...
    Result->SetStringField(TEXT("message"), TEXT("Variable properties updated successfully"));
