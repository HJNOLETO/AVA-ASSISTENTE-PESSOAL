...
O ProjetoGTA já é um projeto **C++** (contém `ProjetoGTA.sln` e `Source/`), então o plugin compila automaticamente ao abrir o `.uproject` no UE5. Não é necessário nenhum passo extra de build.
...
|----------|---------------|---------|
| `ECONNREFUSED :55557` | Plugin não compilou/carregou | Abrir UE5, verificar Edit → Plugins → UnrealMCP → Enabled |
| `Timeout` no `construct_house` | Operação grande (spawn de muitos atores) | Aumentar `UE_MCP_LARGE_TIMEOUT_MS` |
| Plugin não aparece no UE5 | Projeto pode ser Blueprint-only | Verificar se `ProjetoGTA.sln` e `Source/` existem |
| Erro de compilação C++ | Versão do UE5 incompatível | O plugin foi feito para UE 5.5+. Verificar `UnrealMCP.uplugin` |
