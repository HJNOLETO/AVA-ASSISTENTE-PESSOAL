...
interface FileAttachment {
  ...
  content?: string; // base64 for images/binary
  textContent?: string; // extracted text for code/docs
  size: number;
...
const getFileCategory = (filename: string, mimeType: string): FileCategory => {
  ...
  // Documents
  if (["pdf", "doc", "docx", "txt", "rtf", "odt", "odf"].includes(ext))
    return "document";
  if (
    [
      "application/pdf",
...
function useConversationMemory(conversationId: number) {
  ...
  // Gerar resumo (simulação - em produção usarIA)
  const generateSummary = useCallback(
    async (messages: Message[]) => {
      // Aqui você poderia chamar uma API para resumir
      // Por enquanto, mantemos os tópicos principais
...
function useTextToSpeech() {
  ...
  const [voiceIndex, setVoiceIndex] = useLocalStorage<number>(
    "ava-tts-voice-index",
    -1
  );
...
// Hook de reconhecimento de voz corrigido
...
// VAD corrigido - agora acumula chunks corretamente
function useVoiceActivityDetection(options: {
  ...
  // Refs cruciais para manter estado entre ciclos do interval
  ...
  const start = useCallback(async () => {
    try {
      ...
      // Acumular TODOS os chunks - filtragem é feita na lógica de segmento
      ...
      // Iniciar com timeslice baixo para ter granularidade
      recorder.start(100);
      ...
      intervalRef.current = window.setInterval(() => {
        ...
        if (rms > threshold) {
          ...
          if (!speakingRef.current) {
            // Início de novo segmento de fala
            speakingRef.current = true;
            currentSpeechStartRef.current = now;

            // Limpar chunks antigos se houver muito silêncio antes (> 5s)
          ...
          speakingRef.current &&
          now - lastSpeechRef.current > silenceMs
        ) {
          // Fim de segmento por silêncio
          const duration = currentSpeechStartRef.current
            ? now - currentSpeechStartRef.current
            : 0;
          ...
          if (
            duration >= minSpeechMs &&
            currentSegmentChunksRef.current.length > 0
          ) {
            // Criar blob do segmento
            const blob = new Blob(currentSegmentChunksRef.current, {
              type: mimeType,
            });
            const blobSize = blob.size;

            // Só enviar se tiver tamanho mínimo (evitar cliques/spikes)
            if (blobSize > 1000) {
              options.onSegment(blob);
            }

            // Limpar para próximo segmento
            currentSegmentChunksRef.current = [];
          }
        }
      }, 50); // Verificar a cada 50ms para responsividade
    } catch (error) {
  ...
  const stop = useCallback(() => {
    ...
    // Processar segmento pendente se houver fala em andamento
...
const validateAudioMime = (type: string): AudioMime => {
  ...
  return supported.includes(type as AudioMime)
    ? (type as AudioMime)
    : "audio/webm";
};
...
export function AVAChatBox({ conversationId, mode }: AVAChatBoxProps) {
  ...
  const [ollamaBaseUrl, setOllamaBaseUrl] = useLocalStorage<string>(
    "ava-ollama-base-url",
    ""
  );
  const [ollamaAuthToken, setOllamaAuthToken] = useLocalStorage<string>(
    "ava-ollama-auth-token",
    ""
  );
  const [useWebSpeech, setUseWebSpeech] = useLocalStorage<boolean>(
    "ava-use-webspeech",
    false
  );
  const [autoSendOnSilence, setAutoSendOnSilence] = useLocalStorage<boolean>(
    "ava-autosend-silence",
    true
  );
  const [autoSendTyping, setAutoSendTyping] = useLocalStorage<boolean>(
    "ava-autosend-typing",
    false
  );
  const [autoSendTypingDelay, setAutoSendTypingDelay] = useLocalStorage<number>(
    "ava-autosend-typing-delay",
    1500
  );
  const [continuousListening, setContinuousListening] =
    useLocalStorage<boolean>("ava-continuous-listen", false);
  const [contextWindowSize, setContextWindowSize] = useLocalStorage<number>(
    "ava-context-window",
    10
  );
  ...
  // Verificar se há imagens sem suporte
  const hasImages = attachments.some(a => a.category === "image");
  const hasUnsupportedImages =
    hasImages && capabilities && !capabilities.supportsVision;
      ...
      try {
        ...
        for (const file of files) {
          ...
          if (category === "image") {
            ...
            category === "code" ||
            (category === "data" && file.name.endsWith(".csv")) ||
            file.size < 1024 * 1024
          ) {
            try {
        ...
        const immediateAttachments = newAttachments.filter(
          a => !a.isProcessing
        );
  ...
  // Handlers de mensagem com contexto
  const handleSendMessage = useCallback(
    async (overrideText?: string) => {
      ...
      // Preparar contexto (últimas N mensagens)
      ...
      // Adicionar resumo se existir e for relevante
      ...
      try {
        const response = await sendMessageMutation.mutateAsync({
          ...
          ...(provider === "ollama" && ollamaBaseUrl.trim()
            ? { ollamaBaseUrl }
            : {}),
          ...(provider === "ollama" && ollamaAuthToken.trim()
            ? { ollamaAuthToken }
            : {}),
          attachments: attachmentsToSend,
          context: contextPayload, // Enviar contexto para o backend
        });
        ...
        // Gerar resumo a cada 10 mensagens
        ...
        setMessages(prev => [
          ...prev,
          {
            ...
            role: "system",
            content:
              "Erro ao enviar mensagem. Verifique sua conexão ou tente novamente.",
            createdAt: new Date(),
          },
        ]);
      } finally {
  ...
  // Aceitar todos os tipos de arquivo relevantes
          ...
          <div className="flex items-center gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-80" sideOffset={8}>
                ...
                >
                  <DropdownMenuRadioItem value="ollama">
                    Ollama
                  </DropdownMenuRadioItem>
                  <DropdownMenuRadioItem value="forge">
                    Forge
                  </DropdownMenuRadioItem>
                </DropdownMenuRadioGroup>
                  ...
                  <>
                    ...
                    <div className="px-2 py-1.5 text-xs space-y-1 text-muted-foreground">
                      <div className="flex justify-between">
                        ...
                        <span
                          className={
                            capabilities.supportsVision ? "text-green-600" : ""
                          }
                        >
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>
                          {capabilities.supportsTools ? "Sim" : "Não"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Thinking:</span>
                        <span>
                          {capabilities.supportsThinking ? "Sim" : "Não"}
                        </span>
                      </div>
                    </div>
                  </>
                ...
                {memory && (
                  <div className="px-2 py-1.5 bg-muted rounded text-xs space-y-1">
                    ...
                    <p className="line-clamp-2">
                      {memory.summary ||
                        `${memory.messageCount} mensagens na memória`}
                    </p>
                  </div>
                )}
                    ...
                    <DropdownMenuSubContent>
                      <div className="px-2 py-1.5">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          {autoSendTypingDelay}ms (
                          {(autoSendTypingDelay / 1000).toFixed(1)}s)
                        </p>
                      </div>
                    </DropdownMenuSubContent>
                  </DropdownMenuSub>
          ...
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-muted-foreground space-y-2">
              ...
              <p className="text-xs text-center max-w-sm">
                Dica: Use o microfone para entrada de voz ou anexe arquivos para
                análise.
                {hasContext &&
                  " A AVA está mantendo o contexto das conversas anteriores."}
              </p>
            </div>
          ) : (
      ...
      <div className="border-t border-border p-4 bg-card/50 backdrop-blur-sm space-y-3">
        ...
        {hasUnsupportedImages && (
          <div className="flex items-center gap-2 text-xs text-destructive bg-destructive/10 p-2 rounded">
            ...
            <span>
              O modelo atual não suporta visão. As imagens podem ser ignoradas.
            </span>
          </div>
        )}
        ...
        <div className="flex gap-2 items-end">
          ...
          <Button
            variant={
              isRecording || continuousListening ? "destructive" : "outline"
            }
            size="icon"
            onClick={
              isRecording
            ...
            title={
              isRecording
                ? "Parar gravação"
                : continuousListening
                  ? "Escuta ativa (clique para parar)"
