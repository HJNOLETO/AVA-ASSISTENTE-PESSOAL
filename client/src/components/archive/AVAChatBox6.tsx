...
const getFileCategory = (filename: string, mimeType: string): FileCategory => {
  ...
  // Code files
  const codeExts = [
    'py', 'js', 'ts', 'tsx', 'jsx', 'html', 'htm', 'css', 'scss', 'sass', 'less',
    'json', 'md', 'markdown', 'yaml', 'yml', 'xml', 'sql', 'java', 'cpp', 'c', 'h',
    'hpp', 'cs', 'go', 'rs', 'php', 'rb', 'swift', 'kt', 'scala', 'r', 'm', 'mat',
    'sh', 'bash', 'zsh', 'fish', 'bat', 'ps1', 'lua', 'perl', 'pl', 'pm'
  ];
...
export function AVAChatBox({ conversationId, mode }: AVAChatBoxProps) {
  ...
  const processTranscription = useCallback(async (audioBlob: Blob) => {
    try {
      ...
      // ✅ CORREÇÃO: Verificar se result existe e é do tipo correto
      if (!result) {
      ...
      // Agora TypeScript sabe que é { text: string }
          ...
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-muted-foreground space-y-2">
              ...
              <p className="text-xs text-center max-w-sm">
                Dica: Use o microfone para entrada de voz ou anexe arquivos para análise.
                {hasContext && " A AVA está mantendo o contexto das conversas anteriores."}
              </p>
            </div>
          ) : (
