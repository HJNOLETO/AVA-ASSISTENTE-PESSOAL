  ...
  chat: router({
    ...
    sendMessage: protectedProcedure
      .input(
        z.object({
          ...
          attachments: z
            ...
            .optional(),
          // ✅ CORREÇÃO 1: Adicionado campo context para memória/conversação
          context: z.object({
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Get LLM response
        try {
          // Build system message with context from previous conversations
          let systemContent = "Você é um assistente virtual inteligente e adaptativo. Responda de forma clara e concisa. Quando analisar código, sugira melhorias específicas e boas práticas. Para documentos, faça resumos ou análises conforme solicitado.";
          
          // ✅ Usar contexto da memória se disponível
  ...
  transcribe: protectedProcedure
    ...
    .mutation(async ({ input }): Promise<{ text: string } | { error: string; code?: string; details?: string }> => {
      try {
        ...
        // ✅ CORREÇÃO: Type Guard - verificar se 'text' existe no resultado
        if (result && typeof result === 'object' && 'text' in result && result.text) {
          ...
          return { 
            ...
            details: "Transcription service returned an error"
          };
        } else {
          return {
