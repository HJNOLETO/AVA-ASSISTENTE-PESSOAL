import { int, sqliteTable, text, integer, index, real, uniqueIndex } from "drizzle-orm/sqlite-core";
...
export const documents = sqliteTable(
  "documents",
  {
    ...
    status: text("status", {
      enum: ["processing", "review", "indexed", "error", "rejected"],
    }).default("processing"),
    ...
    supersededById: integer("supersededById").references((): any => documents.id, { onDelete: "set null" }),
    totalChunks: integer("totalChunks").default(0),
    indexedChunks: integer("indexedChunks").default(0),
    estimatedSizeKB: integer("estimatedSizeKB"),
    pageCount: integer("pageCount"),
    lastAccessedAt: integer("lastAccessedAt", { mode: "timestamp_ms" }),
    retentionDays: integer("retentionDays"),
    embeddingProvider: text("embeddingProvider"),
  ...
  (table) => ({
    ...
    externalIdIdx: index("idx_documents_external_id").on(table.externalId),
    userExternalUniqueIdx: uniqueIndex("uq_documents_user_external").on(table.userId, table.externalId),
  })
);
...
export const documentChunks = sqliteTable(
  ...
  (table) => ({
    ...
    chunkIndexIdx: index("idx_chunks_document_chunk").on(table.documentId, table.chunkIndex),
    docChunkUniqueIdx: uniqueIndex("uq_chunks_document_chunk").on(table.documentId, table.chunkIndex),
  })
);
...
export const appointments = sqliteTable(
  ...
  (table) => ({
    userIdIdx: index("idx_appointments_user_id").on(table.userId),
    customerIdIdx: index("idx_appointments_customer_id").on(table.customerId),
    startDateIdx: index("idx_appointments_start_date").on(table.startTime),
  })
);
...
export const proactiveTasks = sqliteTable("proactiveTasks", {
  ...
  title: text("title").notNull(),
  description: text("description"),
  type: text("type", { enum: ["cron", "one-time", "watcher", "alerta_urgente", "proactive"] }).notNull(),
...
export const legalDeadlines = sqliteTable(
  "legal_deadlines",
  {
    ...
    // Owner of the deadline (consistent with other tables)
    userId: integer("userId")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
...
// ==========================================
// COMMERCIAL & ERP MODULE
// ==========================================
export const products = sqliteTable(
  "products",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    referenceId: text("referenceId"), // ID legado
    name: text("name").notNull(),
  ...
  (table) => ({
    ...
    refIdx: index("idx_products_ref").on(table.referenceId),
  })
);
