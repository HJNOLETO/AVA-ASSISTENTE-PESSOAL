...
- Chunks: 107
- Generated: 2026-04-07T21:22:31.691Z
...
function detectConstructs(text: string, language: string, relativePath: string): FileConstructs {
  ...
  const hooks = uniqueTop(Array.from(tex
```
...
const fnByArrow = Array.from(text.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:async\s*)?\([^)]*\)\s*=>/g)).map((m) => m[1]);
  ...
  const hasSql = /\bselect\b|\binsert\b|\bupdate\b|\bdelete\b|\bfrom\b/i.test(text);
...
```text
tion buildPedagogicalFocus(fileType: EntryType, language: string, constructs: FileConstructs): string {
  if (fileType === 'documentation') {
    return 'Ao estudar, foque em objetivo do modulo, fluxos principais e termos de dominio citados no texto.';
  }
  if (fileType === 'config') {
...
function buildChatMarkdown(chunks: ChunkRecord[], sourcePath: string): s
```

## knowledge-cli.ts (chunk 10)

```text
explanation: buildUtilityExplanation(file, fileType, language, constructs),
    pedagogical_focus: buildPedagogicalFocus(fileType, language, constructs),
    constructs,
  };
}
...
```text
{ file: file.file, chunks: file.chunks, size: file.size });
...
function writeDidacticArtifacts(exportDir: string, sourcePath: string, notes: DidacticNote[]): {
  ...
  const jsonlPath = path.join(exportD
```
...
function writeDidacticArtifacts(exportDir: string, sourcePath: string, notes: DidacticNote[]): {
  ...
  for (const note of notes) {
    ...
    if (note.constructs.hasApiRo
```

## knowledge-cli.ts (chunk 15)
...
async function getOptionsFromPrompt(initial: Partial<CliOptions>): Promise<CliOptions> {
  ...
  return {
    ...
    mode: toMode((modePrompt |
```

## knowledge-cli.ts (chunk 16)

```text
rl.question(`Pasta base de saida [${initial.outRoot ?? path.resolve(process.cwd(), 'knowledge-exports')}]: `);
...
## knowledge-cli.ts (chunk 18)

```text
ing_note: didacticNote.utility_explanation,
      });
    });
